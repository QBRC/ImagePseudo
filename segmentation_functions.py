...
def get_mask_for_slide_image(filePath, display_progress=False, level_to_analyze=None):
    ...
    if level_to_analyze is None:
        ...
        dims_of_selected = level_dims[level_to_analyze]
    ...
    min_pixel_count = min(0.005 * dims_of_selected[0] * dims_of_selected[1], int(np.sum(np.array(mask/255))/10))
...
def get_mask_for_slide_image_tifffile(filePath, display_progress=False):
    from tifffile import TiffFile
    import imagecodecs
    
    slide = open_slide(filePath)
    width_0 = slide.level_dimensions[0][0]
    fp = open(filePath, 'rb')
    tiff = TiffFile(fp)

    for i, page in enumerate(tiff.pages[:-1]):  # discard the last small image 
        print("==================")
        print("page: {}".format(i))
        for tag in page.tags.values():
            ...
            if not name.startswith('Tile'):
                print(name, value)
        if width_0/page.shape[1] > 5 or (page.shape[0] < 5000 and page.shape[1] < 5000):
            # plt.imshow(page.asarray())
            # plt.show()
            break
    
    zoom = width_0 / page.shape[1]
    if display_progress:
        print('Selected image of size (' + str(page.shape[1]) + ', ' + str(page.shape[0]) + ')')
    slide_image = page.asarray()
    if display_progress:
        plt.figure()
        plt.imshow(slide_image)
    fp.close()
    ...
    # Delete small objects
    min_pixel_count = 0.005 * page.shape[0] * page.shape[1]
    mask = np.array(skimage.morphology.remove_small_objects(np.array(mask/255, dtype=bool), min_pixel_count),
                    dtype=np.uint8)
    if display_progress:
        print("Min pixel count: {}".format(min_pixel_count))
...
def parseCSV(csvFile, pattern):
    ...
    for i in df.index.values:
        if df.loc[i,'Label'] == pattern:
            poly_x, poly_y = df.loc[i,'poly_x'].split(','), df.loc[i,'poly_x'].split(',')
            poly_x, poly_y =  list(map(float,poly_x)), list(map(float,poly_y))
            vertices[pattern].append({'X':[], 'Y':[]})
            
            vertices[pattern][-1]['X'].extend(poly_x)
            vertices[pattern][-1]['Y'].extend(poly_y)
...
def createMask(levelDims, vertices, pattern):
    ...
    for i in range(len(vertices[pattern])):
        ...
        rr, cc = polygon(lowX.astype(int), lowY.astype(int), (nRows, nCols))
        mask[rr, cc] = 1
...
def getMask_from_csv(csvFile, svsFile, pattern):
    """ Parses csv File (annotations from I-Viewer) to get mask vertices and returns matrix masks 
    where 1 indicates the pixel is inside the mask, and 0 indicates outside the mask.

    @param: {string} csvFile: name of csv file that contains annotation vertices outlining the mask. 
    @param: {string} svsFile: name of svs file that contains the slide image.
    Returns: slide - openslide slide Object 
             mask - matrix mask of pattern
    """
    vertices = parseCSV(csvFile, pattern) # Parse CSV annotations to get vertices of mask
...
def check_white_patch(image):
    return np.mean(image) > 230
...
'''Get Patches for TiffFile'''
def createMaskTiffFile(levelDims, zoom, vertices, pattern):
    """
    Input: levelDims (nested list): dimensions of each layer of the slide.
           vertices (dict object as describe above)
    Output: (tuple) mask
...
def getMaskTiffFile(xmlFile, svsFile, pattern):
    """ Parses XML File to get mask vertices and returns matrix masks 
    where 1 indicates the pixel is inside the mask, and 0 indicates outside the mask.
    ...
    zoom = width_0 / page.shape[1]
    slide_image = page.asarray()
    mask = createMaskTiffFile([(page.shape[1], page.shape[0])], zoom, vertices, pattern)

    return slide, mask, zoom


# extractPatchByXMLLabeling for TiffFile
def getPatchesTiffFile(slide, mask, zoom, numPatches=0, dims=(0,0), dirPath='', slideNum='', plot=False, plotMask=False, 
              otherPath=None):
    """ Generates and saves 'numPatches' patches with dimension 'dims' from image 'slide' contained within 'mask'.
    @param {Openslide Slide obj} slide: image object
    @param {numpy matrix} mask: where 0 is outside region of interest and 1 indicates within 
    ...
    while i < numPatches:
        ...
        isWhite = checkWhiteSlide(image)
        newPath = otherPath if isWhite else dirPath
        if not isWhite: i += 1
...
'''Example codes for getting patches for TiffFile
for slide_name in slide_names:
    print('Start {}'.format(slide_name))
    slide_file = os.path.join(image_folder, slide_name)
    xml_file = os.path.join(image_folder, slide_name[:-4]+".xml")
    ...
    found = False
    for pattern in patterns:
        outdir = os.path.join(patch_folder, pattern)
        num_patches_generated = len([files for files in os.listdir(outdir)
                                  if re.search(str(slide_name)+'_.+\.png', files) is not None])
        if num_patches_generated >= NUM_PATCHES:
            print(pattern+' existed')
            continue
        else:
            num_patches_to_extract = NUM_PATCHES - num_patches_generated
            
        slide, mask, zoom = getMask(xml_file, slide_file, pattern)
        if slide == 0:
            continue
            
        found = True
        getPatches(slide, mask, zoom, numPatches=num_patches_to_extract, dims=(PATCH_SIZE, PATCH_SIZE), 
                   dirPath=outdir, slideNum=slide_name, plotMask=False, 
                   otherPath=os.path.join(patch_folder, "other"))  # Get Patches
        print(pattern+' done.')
'''
